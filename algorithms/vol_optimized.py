"""V4 Vol-Optimized algorithm — Distribution-based dynamic thresholds.
...
# ---------------------------------------------------------------------------
# Strategy classification sets (same as hedge.py)
# ---------------------------------------------------------------------------
            ...
            for width in widths:
                ...
                if score > best_score:
                    ...
                    best_candidate = TradeSuggestion(
                        ...
                        position_size="Per vol-optimized dynamic sizing",
                        reasoning=[
...
def _build_vol_hedged_strangle(
    chain: OptionChainData,
    analytics: OptionsAnalytics,
    technicals: TechnicalIndicators,
    vol: VolSnapshot,
...
def _compute_vol_lots(
    ...
    elif suggestion.strategy in _VOL_CREDIT:
        net_per_unit = sum(
    ...
    if state.consecutive_losses >= reduce_threshold:
        lots = max(1, lots // 2)
        logger.info("VOL_OPT: %d consecutive losses — reducing to %d lots", state.consecutive_losses, lots)
...
def _apply_vol_fills(
    ...
    if suggestion.strategy in _VOL_CREDIT and net_credit <= 0:
...
def _check_vol_gates(
    ...
    if sell_legs and not buy_legs:
        return "VOL_OPT blocks naked positions — protective legs required"
...
def _check_vol_portfolio_structure(
    state: PaperTradingState,
    suggestion: TradeSuggestion,
    cfg: dict,
) -> str | None:
...
def _check_vol_credit_sl(
    ...
    if position.net_premium > 0 and pnl <= -(abs(position.net_premium) * sl_mult):
        logger.info("VOL_OPT SL: %s loss %.0f >= %.1fx credit %.0f",
    ...
    for leg in position.legs:
        ...
        if delta >= delta_exit:
            logger.info("VOL_OPT SL: delta %.3f >= dynamic exit %.3f on %s %d",
...
def _check_vol_strangle_sl(
    ...
    if position.net_premium > 0 and pnl <= -abs(position.net_premium):
        logger.info("VOL_OPT SL: strangle premium doubled — loss %.0f >= credit %.0f",
    ...
    for leg in position.legs:
        if leg.action == "SELL" and leg.entry_ltp > 0:
            if leg.current_ltp >= 2 * leg.entry_ltp:
                logger.info("VOL_OPT SL: strangle leg %s %d premium doubled (%.1f -> %.1f)",
    ...
    for leg in position.legs:
        ...
        delta = abs(sd.ce_delta if leg.option_type == "CE" else sd.pe_delta)
        if delta >= delta_exit:
            logger.info("VOL_OPT SL: strangle delta %.3f >= exit %.3f",
                         delta, delta_exit)
            return PositionStatus.CLOSED_STOP_LOSS
...
def _check_vol_take_profit(
    ...
    if strategy_type == StrategyType.CREDIT and position.net_premium > 0:
        ...
        if pnl >= target:
            logger.info("VOL_OPT TP: %s pnl %.0f >= %.0f%% of credit (%.0f)",
...
def _enrich_vol_suggestion(
    ...
    width = _compute_spread_width(suggestion.legs)
    is_credit = suggestion.strategy in _VOL_CREDIT
...
@register_algorithm
class VolOptimizedAlgorithm(TradingAlgorithm):
    """V4 Vol-Optimized — Distribution-based dynamic thresholds."""

    name = "vol_optimized"
    display_name = "V4 Vol-Optimized"
    ...
    def generate_suggestions(
        ...
        if vol is None:
            ...
            logger.warning("VOL_OPT: VolSnapshot unavailable, using neutral fallback")
        ...
        logger.info(
            "VOL_OPT regime=%s | pRV=%.2f pVoV=%.2f pVRP=%.2f | delta=%.3f credit=%.3f SL=%.1fx TP=%.0f%% BE=%.2fx",
        ...
        if not macro_override:
            ...
            if near_event and regime == "sell_premium":
                logger.info("VOL_OPT: Macro event within 24h: %s — downgrading to stand_down", event_label)
                regime = "stand_down"

        if regime == "sell_premium":
            # Build IC
            ic = _build_vol_iron_condor(chain, analytics, technicals, vol, cfg)
            if ic is not None:
                liq_reject = _check_vol_liquidity(ic, chain, cfg)
                if liq_reject:
                    rejected.append(ic.model_copy(update={"rejection_reason": liq_reject, "score": 0.0}))
                else:
                    new_legs, fill_reject = _apply_vol_fills(ic, chain, cfg)
                    if fill_reject:
                        rejected.append(ic.model_copy(update={"rejection_reason": fill_reject, "score": 0.0}))
                    else:
                        ic = ic.model_copy(update={"legs": new_legs})
                        enriched = _enrich_vol_suggestion(ic, chain, analytics, technicals, vol, cfg, lots=1)
                        accepted.append(enriched)
                        logger.info("VOL_OPT ACCEPTED: %s | RR=%.2f | POP=%.1f%% | liq=%.0f",
                                    enriched.strategy.value, enriched.risk_reward_ratio,
                                    enriched.pop, enriched.liquidity_score)

            # Build hedged strangle
            hs = _build_vol_hedged_strangle(chain, analytics, technicals, vol, cfg)
            if hs is not None:
                liq_reject = _check_vol_liquidity(hs, chain, cfg)
                if liq_reject:
                    rejected.append(hs.model_copy(update={"rejection_reason": liq_reject, "score": 0.0}))
                else:
                    new_legs, fill_reject = _apply_vol_fills(hs, chain, cfg)
                    if fill_reject:
                        rejected.append(hs.model_copy(update={"rejection_reason": fill_reject, "score": 0.0}))
                    else:
                        hs = hs.model_copy(update={"legs": new_legs})
                        enriched = _enrich_vol_suggestion(hs, chain, analytics, technicals, vol, cfg, lots=1)
                        accepted.append(enriched)
                        logger.info("VOL_OPT ACCEPTED: %s | RR=%.2f | POP=%.1f%% | liq=%.0f",
                                    enriched.strategy.value, enriched.risk_reward_ratio,
                                    enriched.pop, enriched.liquidity_score)
            ...
            ds = _build_vol_debit_spread(chain, analytics, technicals, vol, cfg)
            if ds is not None:
                liq_reject = _check_vol_liquidity(ds, chain, cfg)
                if liq_reject:
                    rejected.append(ds.model_copy(update={"rejection_reason": liq_reject, "score": 0.0}))
                else:
                    new_legs, fill_reject = _apply_vol_fills(ds, chain, cfg)
                    if fill_reject:
                        rejected.append(ds.model_copy(update={"rejection_reason": fill_reject, "score": 0.0}))
                    else:
                        ds = ds.model_copy(update={"legs": new_legs})
                        enriched = _enrich_vol_suggestion(ds, chain, analytics, technicals, vol, cfg, lots=1)
                        accepted.append(enriched)
                        logger.info("VOL_OPT ACCEPTED: %s | RR=%.2f | POP=%.1f%% | liq=%.0f",
                                    enriched.strategy.value, enriched.risk_reward_ratio,
                                    enriched.pop, enriched.liquidity_score)
            ...
            logger.info("VOL_OPT: Stand-down regime — no new suggestions")
    ...
    def evaluate_and_manage(
        ...
        if vol is None:
            vol = _neutral_snapshot()
            logger.warning("VOL_OPT: VolSnapshot unavailable during management, using neutral fallback")
        ...
        for position in state.open_positions:
            ...
            if chain:
                if strategy_type == StrategyType.CREDIT:
                    if position.strategy == StrategyName.SHORT_STRANGLE.value:
                        exit_reason_sl = _check_vol_strangle_sl(position, chain, vol, cfg)
                    else:
                        exit_reason_sl = _check_vol_credit_sl(position, chain, vol, cfg)
                    if exit_reason_sl:
                        exit_reason = exit_reason_sl
                elif strategy_type == StrategyType.DEBIT:
                    exit_reason_sl = _check_vol_debit_sl(position, cfg)
                    if exit_reason_sl:
                        exit_reason = exit_reason_sl

            # Dynamic take profit
            if exit_reason is None:
                tp = _check_vol_take_profit(position, vol, cfg)
                if tp:
                    exit_reason = tp
            ...
            if exit_reason is None and regime == "stand_down" and strategy_type == StrategyType.CREDIT:
                ...
                if pnl > 0:
                    logger.info("VOL_OPT: Regime changed to stand_down — closing profitable credit %s",
                                position.strategy)
            ...
            if exit_reason is None and chain and technicals:
                ...
                if is_credit_range and _is_breakout(technicals):
                    logger.info("VOL_OPT: Breakout detected — closing %s", position.strategy)
            ...
            if exit_reason:
                ...
                logger.info("VOL_OPT CLOSED: %s | reason=%s | pnl=%.2f",
                             record.strategy,
        ...
        if consecutive_losses >= halt_after:
            trading_halted = True
            logger.warning("VOL_OPT HALTED: %d consecutive losses >= %d", consecutive_losses, halt_after)
        ...
        if state.daily_start_capital > 0:
            daily_pnl = account_value - state.daily_start_capital
            if daily_pnl <= -(state.daily_start_capital * daily_loss_pct / 100):
                logger.warning("VOL_OPT SHUTDOWN: daily loss %.0f > %.1f%%", abs(daily_pnl), daily_loss_pct)
                return state
        ...
        if state.weekly_start_capital > 0:
            weekly_pnl = account_value - state.weekly_start_capital
            if weekly_pnl <= -(state.weekly_start_capital * weekly_dd_pct / 100):
                logger.warning("VOL_OPT SHUTDOWN: weekly drawdown %.0f > %.1f%%", abs(weekly_pnl), weekly_dd_pct)
                return state
        ...
        if state.trading_halted:
            logger.info("VOL_OPT: Trading halted — skipping new positions")
            return state
        ...
        for suggestion in suggestions:
            ...
            # Global gates
            gate_reason = _check_vol_gates(state, suggestion, chain, cfg)
            if gate_reason:
                logger.info("VOL_OPT GATE BLOCKED: %s — %s", suggestion.strategy.value, gate_reason)
                continue

            # Portfolio structure
            struct_reason = _check_vol_portfolio_structure(state, suggestion, cfg)
            if struct_reason:
                logger.info("VOL_OPT STRUCTURE BLOCKED: %s — %s", suggestion.strategy.value, struct_reason)
                continue
            ...
            vol_lots, size_reject = _compute_vol_lots(suggestion, state, vol, cfg)
            if size_reject:
                logger.info("VOL_OPT SIZING REJECTED: %s — %s", suggestion.strategy.value, size_reject)
                continue
            ...
            logger.info(
                "VOL_OPT OPENED: %s | lots=%d | margin=%.0f | premium=%.2f | regime=%s",
