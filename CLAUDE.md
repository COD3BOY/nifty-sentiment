...
| 2026-02-25 | `algorithms/jarvis.py` | Add `entry_end_time` gate (default "14:45") in Phase 2; fix cooldown to use `refresh_ts` for simulation compatibility | Jarvis had no entry cutoff on non-expiry days — could open trades right up to 15:30 market close; cooldown used wall clock in simulation |
| 2026-02-25 | `algorithms/jarvis_m.py` (new) | New "V2 Jarvis M" algorithm: Jarvis + full MarketContext wiring — context scoring via V1 (context=, observation=), IV expansion exit, context exits (neutral trend, directional reversal, vol regime shift), vol stand-down, re-entry tracking, score gates, confidence loosening, max trades/day | Jarvis was context-unaware; Jarvis M adds all context-driven features while keeping original Jarvis untouched |
| 2026-02-25 | `algorithms/__init__.py` | Register `jarvis_m` in `discover_algorithms()` | Auto-discovery for new algorithm |
| 2026-02-25 | `config.yaml` | Add `entry_end_time: "14:45"` to jarvis section; add full `jarvis_m` config section (all Jarvis params + context/timing params) | Config for entry cutoff and new algorithm |
